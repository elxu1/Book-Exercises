...
class PGroup {
  ...
  add(element){
	...
	newPGroup.values = [...this.values];
    if(!newPGroup.has(element)){
       newPGroup.values.push(element);
    }
    return newPGroup;
  }
  ...
  delete(element){
    let newPGroup = new PGroup();
    newPGroup.values = this.values.filter(item => item !== element);
    return newPGroup;
  }

  has(element){
    return this.values.some(value => value === element);
  }
